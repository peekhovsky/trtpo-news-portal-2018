...
Project must be simple to use (4-5 buttons and text fields page), and light-weight (should not have lags on non-powerful devices, and should load fast even if user have low-speed connection). Aslo, that is important to have safe security system (it might be hacked by trespassers when it becomes popular).  
